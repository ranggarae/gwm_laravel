    ...
    <div class="col-lg-12 col-ml-12 padding-bottom-30">
        <div class="row">
            ...
            <div class="col-lg-12 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('About Page Management')}}</h4>
                        
                        <ul class="nav nav-tabs" id="aboutManagementTab" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" id="about-section-tab" data-toggle="tab" href="#about-section" role="tab" aria-selected="true">{{__('About Us Section')}}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" id="know-about-tab" data-toggle="tab" href="#know-about" role="tab" aria-selected="false">{{__('Know About Section')}}</a>
                            </li>
                            <li class="nav-item">
                        ...
                        <div class="tab-content margin-top-40" id="aboutManagementTabContent">
                            <!-- TAB 1: About Us Section -->
                            <div class="tab-pane fade show active" id="about-section" role="tabpanel">
                                <form action="{{route('admin.about.page.about')}}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <nav>
                                        <div class="nav nav-tabs language-tabs" id="nav-tab-about" role="tablist">
                                            @foreach($all_languages as $key => $lang)
                                                <a class="nav-item nav-link @if($key == 0) active @endif" id="nav-about-{{$lang->slug}}-tab" data-toggle="tab" href="#nav-about-{{$lang->slug}}" role="tab" aria-selected="true">{{$lang->name}}</a>
                                            @endforeach
                                        </div>
                                    </nav>
                                    <div class="tab-content margin-top-30" id="nav-tabContent-about">
                                        @foreach($all_languages as $key => $lang)
                                            <div class="tab-pane fade @if($key == 0) show active @endif" id="nav-about-{{$lang->slug}}" role="tabpanel">
                                                <div class="form-group">
                                                    <label for="about_page_{{$lang->slug}}_about_section_title">{{__('Title')}}</label>
                            ...
                            <!-- TAB 2: Know About Section -->
                            <div class="tab-pane fade" id="know-about" role="tabpanel">
                                <div class="row">
                                    <!-- Items Table -->
                                    <div class="col-lg-6">
                                        ...
                                        <nav>
                                            <div class="nav nav-tabs language-tabs" id="nav-tab-know" role="tablist">
                                                @php $a=0; @endphp
                                                @foreach($all_know_about_items as $key => $item)
                                                        <a class="nav-item nav-link @if($a == 0) active @endif"  data-toggle="tab" href="#known_about_tab_{{$key}}" role="tab">{{get_language_by_slug($key)}}</a>
                                                    @php $a++; @endphp
                                                @endforeach
                                            </div>
                                        </nav>
                                            ...
                                            @foreach($all_know_about_items as $key => $item)
                                                <div class="tab-pane fade @if($b == 0) show active @endif" id="known_about_tab_{{$key}}" role="tabpanel" >
                                                    <div class="table-wrap table-responsive">
                                                        <table class="table table-default know-about-table">
                                                            <thead>
                                                            <th>{{__('Title')}}</th>
                                                            <th>{{__('Image')}}</th>
                                                            <th>{{__('Action')}}</th>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    @if (!empty($know_section_img))
                                                                        <div class="attachment-preview">
                                                                            <div class="thumbnail">
                                                                                <div class="centered">
                                                                                    <img class="avatar user-thumb" src="{{$know_section_img['img_url']}}" alt="" style="max-width: 50px;">
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                    ...
                                    <div class="col-lg-6 border-left">
                                        ...
                                        <form action="{{route('admin.about.know')}}" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <nav>
                                                <div class="nav nav-tabs language-tabs" role="tablist">
                                                    @foreach($all_languages as $key => $lang)
    ...
    <script>
        $(document).ready(function () {
            ...
            $('a[data-toggle="tab"][id$="-tab"]').on('show.bs.tab', function(e) {
                localStorage.setItem('activeAboutTab', $(e.target).attr('href'));
            });
            var activeTab = localStorage.getItem('activeAboutTab');
            if(activeTab){
                $('#aboutManagementTab a[href="' + activeTab + '"]').tab('show');
            }

            $('.know-about-table').DataTable( {
                "order": [[ 0, "desc" ]]
            } );
